...
$nilai = mysqli_query($mysqli, "SELECT * FROM kelas t1, kelas_ujian t2 WHERE t1.id_kelas=t2.id_kelas AND t2.id_ujian='$r[id_ujian]'");
 ...
 $siswa = mysqli_query($mysqli, "SELECT * FROM siswa WHERE id_kelas='$l[id_kelas]'");
 $jmlsiswa = mysqli_num_rows($siswa);
echo " <a href=?module=nilaiop&act=kelas&id=$r[id_ujian]&kls=$l[id_kelas] class='btn btn-info'><i class='fa  fa-server'></i> $l[nama_kelas]  ($jmlsiswa)</a>";
 }
echo"  </td></tr>";
